...
class WallFollower(Node):
    ...
    def control_timer_callback(self,):
        ...
        # 2. if the forward distance to the wall is more than 0.5m
        #    continue moving forward
        elif self.current_state == 'forward':
            if self.distance_array[0] < 0.5:
                self.current_state = 'turn_right'
                self.target_yaw = self.current_yaw + np.deg2rad(self.turn_left_deg)
            else:
        ...
        # 3. try keep turn right until it reaches target yaw
